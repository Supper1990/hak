...
def main() -> None:
    print(
        """
Эта программа демонстрирует работу простой полносвязной нейросети.\n
Она принимает пары битов на входе и обучается воспроизводить их XOR: результат равен 1, если биты отличаются, и 0, если совпадают.\n
Во время выполнения сеть многократно прогоняет данные, оценивает ошибку и корректирует веса, чтобы научиться выдавать правильные ответы.
""".strip()
    )
